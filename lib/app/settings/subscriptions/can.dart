...
class ServerHealthCheck extends StatelessWidget {
  ...
  static Future<void> checkServerHealthAndNavigate(BuildContext context) async {
    try {
      final response = await http.get(Uri.parse('${apiEndpoint}/subscriptions/can-i-subscribe/'));
      print("WHAT DOES GANDALF SAYS? ---------> ${response.body}");
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('O serviço de assinaturas não está disponível atualmente. Tente novamente mais tarde.')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
